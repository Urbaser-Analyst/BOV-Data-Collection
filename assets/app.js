...
function renderAssignTable() {
  ...
  rows.forEach((a) => {
    const draft = state.drafts[a.key] || {
      wet: a.wet, dry: a.dry, sanitary: a.sanitary, dhw: a.dhw
    };
    ...
    tr.innerHTML = `
      <td><span class="status-badge ${a.status}">${a.status === 'completed' ? '✓ Completed' : '◷ Pending'}</span></td>
  ...
  tbody.querySelectorAll('.row-submit-btn').forEach(btn => {
...
function readRowValues(key) {
  const wrapRow = document.querySelector(`.assign-table tr[data-key="${CSS.escape(key)}"]`);
...
function flashRows(keys) {
  keys.forEach(key => {
    const row = document.querySelector(`.assign-table tr[data-key="${CSS.escape(key)}"]`);
    if (row) {
...
$('submitAllBtn').addEventListener('click', () => {
  ...
  const keysWithData = state.assignments
    .filter(a => {
      ...
      if (!d) return a.status !== 'completed' ? false : false; // rows with no drafts & not yet touched: skip unless already completed and unedited
